...
def get_hype(df, grouping, weights=(1,1,1), delta_t=1):
    ...
    for group in grouping:
        ...
        for key in keys:
            tweets_group = pd.concat([tweets_group,df[df.keyword == key].copy()])
...
def combine_hype_to_csv(tweets_df, grouping):

    tweets_df.timestamp = round_time(tweets_df.timestamp, freq='D')

    hype = get_hype(tweets_df, grouping)
...
# combine_hype_to_csv(tweets, grouping)